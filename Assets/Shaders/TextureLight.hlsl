...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	ComputeSpotLight(Mater, SpLight2, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputeSpotLight(Mater, SpLight3, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
    ...
    float FogStart = fog.start; // anything closer than this value will have no fog (5)
    float FogRange = fog.range; // fog contribution increases linearly until full fog at dist = FogStart + Range (25)
